...
import json, uuid, os, datetime, hashlib, sys, secrets, string, threading, time
from urllib.parse import urlparse, parse_qs

# ── アクティブセッション管理（同時実行ユーザー数の把握用）──────────────
_sessions_lock   = threading.Lock()
_active_sessions = {}   # session_id -> started_at (float)
SESSION_EXPIRE_S = 600  # 10分応答なしで自動削除

def _cleanup_sessions():
    """期限切れセッションを削除（各操作時に呼ぶ）"""
    now = time.time()
    expired = [k for k, v in _active_sessions.items() if now - v > SESSION_EXPIRE_S]
    for k in expired:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == '/api/session/count':
            self._send_json(200, {'active': session_count()})
        elif path == '/health':
    ...
    def do_POST(self):
        ...
        if path == '/api/save':
            ...
            sid = session_start()
            self._send_json(200, {'session_id': sid, 'active': session_count()})
            ...
            try:
                ...
                sid = ''
            session_end(sid)
            self._send_json(200, {'ok': True, 'active': session_count()})
        elif path == '/api/session/heartbeat':
            try:
                sid = json.loads(body).get('session_id', '')
            except Exception:
                sid = ''
            session_heartbeat(sid)
            self._send_json(200, {'active': session_count()})
